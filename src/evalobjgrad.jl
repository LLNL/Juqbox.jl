...
#    tik1         ::Float64
    ...
    # Add starting and ending indices for B-spline coefficients in each interval
    d1_start:: Vector{Int64}
    d1_end:: Vector{Int64}
...
# constructor for regular arrays (full matrices)
    function objparams(Ne::Array{Int64,1}, Ng::Array{Int64,1}, T::Float64, nsteps::Int64;
        ...
        d1_start = zeros(Int64, nTimeIntervals)
        d1_end = zeros(Int64, nTimeIntervals)
        ...
        if nTimeIntervals == 1
            ...
            # starting and ending indices for the B-spline coefficients
            dtknot = T/(D1-2)
            # test
            # tc = dtknot*( collect(1:D1) .- 1.5)
            # println("Tcenter: ", tc)
            # for k = 1:D1
            #     println("B-idx = ", k, " support: (", tc[k] - 1.5*dtknot, ", ", tc[k] + 1.5*dtknot, "]")
            # end
                
            for q = 1: nTimeIntervals-1
                k0 = ceil(Int64, T0int[q]/dtknot)
                k1 = 3 + floor(Int64, T0int[q+1]/dtknot)
                d1_start[q] = max(1, k0)
                d1_end[q] = min(D1, k1)
                # println("Interval = ", q, " support = (", T0int[q], ", ", T0int[q+1], "]")
            end
            q = nTimeIntervals
            k0 = ceil(Int64, T0int[q]/dtknot)
            d1_start[q] = max(1, k0)
            d1_end[q] = D1
            # println("Interval = ", q, " support = (", T0int[q], ", ", T, "]")
        end
...
function traceobjgrad(pcof0::Array{Float64,1},  p::objparams, verbose::Bool = false, evaladjoint::Bool = true)
    ...
    infid = (1.0-tracefidabs2(w.vr, -w.vi, p.Utarget_r, p.Utarget_i)) #std def of infidelity
    ...
    elseif p.pFidType == 2 # infidelity
        primaryobjf = infid # insensitive to global phase angle
    ...
    elseif p.pFidType == 4 # Generalized Infidelity (convex)
        primaryobjf = (norm(w.vr)^2 + norm(w.vi)^2)/p.N - tracefidabs2(w.vr, -w.vi, p.Utarget_r, p.Utarget_i)
    # elseif p.pFidType == 4 # OLD objective
    ...
    if verbose
        ...
        mfidelityrot = 1.0 - infid
        println("Final (std) trace infidelity = ", infid, " trace fidelity = ", mfidelityrot)
        if p.pFidType != 2
            println("NOTE: the std trace fidelity is different from the primary objective")
        end
...
function lagrange_grad(pcof0::Array{Float64,1},  p::objparams, objf_grad::Vector{Float64}, verbose::Bool = true)
    ...
    evaladjoint = true
    objf_grad[:] .= 0.0 # initialize the VALUES pointed to by 'objf_grad' to zero
        ...
        if eval1gradient # test 1 component of the gradient
            ...
            if interval < p.nTimeIntervals
                ...
                elseif p.pFidType == 4 # Generalized infidelity
                    # test gradient wrt control parameter p.kpar
                    salpha1 = tracefidcomplex(dUda_r, dUda_i, p.Utarget_r, p.Utarget_i) # scaled by 1/N
                    fnrm_alpha1 = tracefidreal(dUda_r, dUda_i, Uend_r, Uend_i) # scaled by 1/N
                    dFda_kpar = 2*fnrm_alpha1 - 2*real(conj(scomplex0)*salpha1) # gradient of generalized infidelity
                end
                grad_kpar += dFda_kpar
...
################################################
function final_grad(pcof0::Array{Float64,1},  p::objparams, objf_grad::Vector{Float64}, eval1gradient::Bool = true)
    # shortcut to working_arrays object in p::objparams
    w = p.wa
    ...
    if eval1gradient
        println("final_grad: Vector dim Ntot =", p.Ntot , ", Guard levels Nguard = ", p.Nguard , ", Param dim, Psize = ", p.nCoeff, ", Spline coeffs per func, D1= ", p.D1, " Tikhonov coeff: ", p.tik0)
    ...
    if eval1gradient
        println("Final time: ", p.T, ", total number of time steps: " , p.nsteps , ", time step: " , dt )
        println("final_grad: length(pcof) =  ", length(pcof0), " nAlpha = ", p.nAlpha, " nWinit = ", p.nWinit)
    end
    ...
    if eval1gradient # test 1 component of the gradient
        ...
        # gradient with respect to the initial condition for the last time interval (W)
        if interval == interval_kp+1 # NOTE: interval = p.nTimeIntervals
            # p = row_kp, q = col_kp
            if p.pFidType == 1
                v_rq = Uend_r[:, col_kp] - p.Utarget_r[:, col_kp]
    ...
    grad_no_tik = objf_grad[p.kpar]
    tikhonov_grad!(alpha, p, objf_grad) # in-place addition of the
    # note: the Tikhonov gradient also needs to be added to grad_kpar
    grad_kpar += (objf_grad[p.kpar] - grad_no_tik)
...
function unitary_jacobian(pcof0::Vector{Float64}, jac_e::Vector{Float64}, p::objparams, verbose::Bool = true)
    ...
    # In-place assignment of an array only works with the syntax jac_e[:] = vector or jac_e[nJac] = element
    # The statement jac_e = vector changes the pointer to jac_e within the function, but the result is not available in the calling function
...
function state_jacobian(pcof0::Vector{Float64}, jac_e::Vector{Float64}, rows::Vector{Int32}, cols::Vector{Int32}, p::objparams, verbose::Bool = false)
    ...
    for interval = 1:p.nTimeIntervals-1 # loop over the state constraints for each time interval
        ...
        #####################################
        # UPDATE NEEDED HERE: Replace the adjoint solver by forward evaluation wrt each 
        # component of the B-spline control vector
        ##################################### 
...
##################################################
function c2norm_constraints(pcof0::Vector{Float64}, e_con::Vector{Float64}, p::objparams, verbose::Bool = false)
        # shortcut to working_arrays object in p::objparams  
    w = p.wa
...
#  Evaluate Jacobian of the unitary constraints
##################################################
function c2norm_jacobian(pcof0::Vector{Float64}, jac_e::Vector{Float64}, p::objparams, verbose::Bool = false)
    if p.nTimeIntervals == 1
        # No constraints
        return nothing
    end
...
#  Get row/col indexing for the Jacobian of the unitary constraints
##################################################
function c2norm_jacobian_idx(rows::Vector{Int32}, cols::Vector{Int32}, p::objparams, verbose::Bool = false)
    if p.nTimeIntervals == 1
        # No constraints
        return nothing
    end
    ...
    if verbose
        println("In c2norm_jacobian_idx()")
    end
    # shortcut to working_arrays object in p::objparams  
    w = p.wa
    ...
    for interval = 1:p.nTimeIntervals-1
        ...
        idxr = nJac .+ (1:p.nAlpha) # index range to assign
        if verbose
            ("Cons # ", cons_idx, " Assigning jac wrt alpha, index range = ", idxr, " cols = ", 1:p.nAlpha)
        end
        rows[idxr] .= cons_idx 
        cols[idxr] = 1:p.nAlpha
        #jac_e[idxr] = quadGrad[1:p.nAlpha]
