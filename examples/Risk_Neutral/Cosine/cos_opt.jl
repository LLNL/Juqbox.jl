...
if(!@isdefined(dispersion_values))
...
Nguard = 3 # Number of guard/forbidden energy levels
...
# Build the Hadamard
H = Matrix{ComplexF64}(I, Ntot, Ntot)
H[3,3] = sqrt(0.5)
H[3,4] = sqrt(0.5)
H[4,3] = sqrt(0.5)
H[4,4] = -sqrt(0.5)

# Perform a |0⟩ → (|2⟩ + |3⟩)/√2 transformation
utarget = H*utarget
...
startFile="cos-pcof-opt.jld2" # "swap02-pert-4em1-pcof-opt.jld2"
...
if nquad == 1
  maxctrl = 2*pi*15.0e-3
else 
  maxctrl = 2*pi*20.0e-3
end
...
if startFromScratch
  D1 = 400 # Number of B-spline coefficients per frequency, sin/cos and real/imag
  if Nfreq > 1
...
prob = Juqbox.setup_ipopt_problem(params, wa, nCoeff, minCoeff, maxCoeff, maxIter=maxIter, lbfgsMax=lbfgsMax, startFromScratch=startFromScratch, ipTol=ipTol,acceptTol=acceptTol, acceptIter=acceptIter, nodes=nodes, weights=weights, dispersion_values=dispersion_values)
