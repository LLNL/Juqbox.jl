...
Ng = [2,2,2,2] # Number of extra guard levels
f01 = [4.1, 4.4, 4.2, 4.6]
favg = sum(f01)/2
rot_freq = copy(f01)
xi = [-0.22, -0.225, -0.23, -0.235]
xi12 = -[0.1, 0, 0, 0.08, 0, 0.12] # order: x12, x13, x14, x23, x24, x34
couple_type = 1 # 1: cross-kerr (negative coeffs), 2: Jaynes-Cummings (positive coeffs)
# assign the target gate
N = prod(Ne)
target_gate = get_swap_1d_gate(4) # get_swap_13_1()
...
ncores = 4 # for Chebyshev # prod(Ne)
